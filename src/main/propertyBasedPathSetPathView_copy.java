...
public class propertyBasedPathSetPathView_copy {
...
//            stmt.executeQuery("DROP INDEX ldbc01.previdIndex;");
    ...
    public static void VML_declare(Statement stmt){
        try {
            ...
            stmt.executeQuery("SELECT * from cypher('ldbc01', " +
                    "$$ MATCH p=(c:Comment)-[e1:REPLY_OF]->(m:Comment)<-[e2:LIKES]-(s:Person) WHERE NOT m.browserUsed = 'Internet Explorer' AND  id(c) <845100000000000  " +
                    " WITH DISTINCT c,m,s,e1,e2 CREATE (c)-[rr1:VML{coID:ID(e1)}]->(m)-[rr2:VML{coID:ID(e2)}]->(s)" +
                    "WITH c,rr1,rr2 SET rr2.prevID=ID(rr1) "+
                    " $$) as (e1 agtype);");

            stmt.executeQuery("SELECT * from cypher('ldbc01', " +
                    "$$ MATCH (c:Comment)-[e1:REPLY_OF]->(m:Comment)<-[e2:LIKES]-(s:Person) WHERE NOT m.browserUsed = 'Internet Explorer' AND  id(c) <845100000000000 " +
                    " WITH DISTINCT c " +
