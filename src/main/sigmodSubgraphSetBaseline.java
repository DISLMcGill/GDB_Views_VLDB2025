...
public class sigmodNodeSetBaseline {
    ...
    public static void VCS_S_baseline(Statement stmt){
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * from cypher('ldbc1', " +
                    ...
                    " RETURN DISTINCT m $$) as (idpo agtype);");
    ...
    public static void VCS_L_baseline(Statement stmt){
        try {
            int count = 0;
            ResultSet rs = stmt.executeQuery("SELECT * from cypher('ldbc1', " +
                    "$$ MATCH p=(c:Comment)-[:REPLY_OF]->(m:Comment)<-[:LIKES]-(s:Person)<-[:HAS_CREATOR]-(t:Post) WHERE id(s) <1125899906847000 AND id(c) <850600000000000 " +
                    " RETURN DISTINCT m $$) as (idpo agtype);");
    ...
    public static void VCL_S_baseline(Statement stmt){
        try {
            int count = 0;
            ResultSet rs = stmt.executeQuery("SELECT * from cypher('ldbc1', " +
                    "$$ MATCH p=(c:Comment)-[:REPLY_OF]->(m:Comment)<-[:LIKES]-(s:Person)<-[:HAS_CREATOR]-(t:Post) WHERE id(s) <1125899906853000 AND id(c) <846500000000000" +
                    " RETURN DISTINCT m $$) as (idpo agtype);");
    ...
    public static void VCL_L_baseline(Statement stmt){
        try {
            int count = 0;
            ResultSet rs = stmt.executeQuery("SELECT * from cypher('ldbc1', " +
                    "$$ MATCH p=(c:Comment)-[:REPLY_OF]->(m:Comment)<-[:LIKES]-(s:Person)<-[:HAS_CREATOR]-(t:Post) WHERE id(s) <1125899906853000 AND id(c) <847200000000000" +
                    " RETURN DISTINCT m $$) as (idpo agtype);");
