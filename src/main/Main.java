...
public class Main {
    ...
    protected static Map<String,List<List<Integer>>> recordTable = new ConcurrentHashMap<>();
    ...
    protected static Map<String,HashMap<String, HashMap<String,Integer>>> nodeCountTable = new ConcurrentHashMap<>();
    protected static Map<String,HashMap<String, HashMap<String,Integer>>> edgeCountTable = new ConcurrentHashMap<>();
    ...
    private static void experiment_terminal(String command) {
        ...
        try {
            if (!command.startsWith("CREATE VIEW") && command.startsWith("CREATE")) {
                boolean insertAnEdge = checkInsertType(command);
                if(insertAnEdge)
                    updateViewWhenInsertingAnEdge(command);
                else //insert a node
                    updateViewWhenInsertingANode(command);
            }else if (command.startsWith("original")) {
                String query = command.split("original")[1];
                connector.executeDirectly(query,myWriter);
            } else if (!command.startsWith("CREATE") && command.contains("CREATE")){
                // assume it's an edge update, need to add some
                boolean insertAnEdge = checkInsertType(command);
                if(insertAnEdge)
                    updateViewWhenInsertingAnEdge(command);
                else //insert a node
                ...
                // assume it's an edge update, need to add some validation
                boolean insertAnEdge = checkInsertType(command);
                if(insertAnEdge)
                    updateViewWhenDeletingAnEdge(command);
                else //insert a node
                    updateViewWhenDeletingANode(command);
		    ...
		    myWriter.write(total + ",");
                    ...
                    if(partNum >  2) {
                        ...
                        if(subCommand.contains("rec")){
                            //add another processUseViewRecord for table views only
                            processUseViewRecord(subCommand);
                        }else
    ...
    public static boolean checkInsertType(String command){
        String varName = "";
        boolean isEdge = true;
        String remainingQuery = "";
        if(command.contains("DELETE")){
            remainingQuery = command.split("DELETE")[0];
            varName = command.split("DELETE")[1].trim();
        } else {
            remainingQuery = command.split("RETURN")[0];
            varName = command.split("RETURN ID\\(")[1].split("\\)")[0].trim();
        }

        while(remainingQuery.contains("(")){
            String nameAndType = remainingQuery.split("\\(")[1].split("\\)")[0];
            if(nameAndType.contains("{")) nameAndType = nameAndType.split("\\{")[0];
            if(nameAndType.contains(":")) nameAndType = nameAndType.split(":")[0];
            if(nameAndType.contains(varName)) {isEdge= false;break;}
            remainingQuery = remainingQuery.substring(remainingQuery.indexOf(")")+1);
        }
    ...
    public static void updateViewWhenInsertingANode(String command){
        long start = System.currentTimeMillis();
        String newNodeType = command.split("CREATE")[1].split("\\(")[1].split("\\)")[0].split(":")[1];
        if(newNodeType.contains("{")) newNodeType = newNodeType.split("\\{")[0];
        String newNodeId = connector.updateAnEntity(command);

        String findNewMatchQuery = "";
        if(Objects.equals(newNodeId, ""))  return;

        for(Map.Entry<String, String> entry : viewQueryTable.entrySet()) {
            String viewName = entry.getKey();
            String viewDeclarationQuery = entry.getValue();
            List<String> variableNamesForTheNodeType = findVariableNamesByNodeType(viewDeclarationQuery, newNodeType);

            if (Objects.equals(typeTable.get(viewName), "NODE")) {
                for (String nodeName : variableNamesForTheNodeType) {
                    findNewMatchQuery = viewDeclarationQuery + " AND ID(" + nodeName + ")=" + newNodeId +
                            " RETURN DISTINCT ID(" + viewReturnVarTable.get(viewName) + ")";
                    HashMap<String, List<Integer>> newNodeids = connector.executeQuery(findNewMatchQuery);
                    if (!newNodeids.isEmpty()) {
                        int newCountUnderAllnames = 0;
                        for (Map.Entry<String, List<Integer>> additionalNodeSet : newNodeids.entrySet()) {
                            String varName = additionalNodeSet.getKey();
                            List<Integer> newSet = additionalNodeSet.getValue();
                            List<Integer> originalset = nodeTable.get(viewName).get(varName);
                            originalset.removeAll(newSet);
                            originalset.addAll(newSet);
                            nodeTable.get(viewName).put(varName, originalset);
                            newCountUnderAllnames += originalset.size();
                            System.out.println("updated size: " + newCountUnderAllnames);
                        }
                    }
                    long end = System.currentTimeMillis();
                    System.out.println("updated duration: " + (end - start));
    ...
    public static void updateViewWhenDeletingANode(String command){
        long start = System.currentTimeMillis();
        String removedNodeName = command.split("DELETE")[1].trim();
        String removedNodeType = command.split(removedNodeName+":")[1].split("\\)")[0];
        String nodeIdtoRemove = connector.updateAnEntity(command.replace("DELETE","RETURN ID(") + ")");
        if(removedNodeType.contains("{")) removedNodeType = removedNodeType.split("\\{")[0];
        String findInvalidNodeQuery = "";
        if(Objects.equals(nodeIdtoRemove, ""))  return;

        for(Map.Entry<String, String> entry : viewQueryTable.entrySet()) {
            String viewName = entry.getKey();
            String viewDeclarationQuery = entry.getValue();
            List<String> variableNamesForTheNodeType = findVariableNamesByNodeType(viewDeclarationQuery, removedNodeType);

            if (Objects.equals(typeTable.get(viewName), "NODE")) {
                for (String nodeName : variableNamesForTheNodeType) {
                    findInvalidNodeQuery = viewDeclarationQuery + " AND ID(" + nodeName + ")=" +nodeIdtoRemove +
                            " RETURN DISTINCT ID(" + viewReturnVarTable.get(viewName) + ")";
                    HashMap<String, List<Integer>> nodesToRemove = connector.executeQuery(findInvalidNodeQuery);
                    if (!nodesToRemove.isEmpty()) {
                        int newCountUnderAllnames = 0;
                        for (Map.Entry<String, List<Integer>> additionalNodeSet : nodesToRemove.entrySet()) {
                            String varName = additionalNodeSet.getKey();
                            List<Integer> removeSet = additionalNodeSet.getValue();
                            List<Integer> originalset = nodeTable.get(viewName).get(varName);
                            originalset.removeAll(removeSet);
                            nodeTable.get(viewName).put(varName, originalset);
                            newCountUnderAllnames += originalset.size();
                            System.out.println("updated size: " + newCountUnderAllnames);
                        }
                    }
                    connector.executeQuery("MATCH (n) WHERE ID(n)="+ nodesToRemove + " DELETE n");
                    long end = System.currentTimeMillis();
                    System.out.println("updated duration: " + (end - start));
    ...
    public static void updateViewWhenInsertingAnEdge(String command){
        long start = System.currentTimeMillis();
        String newEdgeType = command.split("CREATE")[1].split("\\[")[1].split("]")[0].split(":")[1];
        String newEdgeId = connector.updateAnEntity(command);
        String findNewMatchQuery = "";
        if(Objects.equals(newEdgeId, ""))  return;

        for(Map.Entry<String, String> entry : viewQueryTable.entrySet()){
            String viewName = entry.getKey();
            String viewDeclarationQuery = entry.getValue();
            List<String> variableNamesForTheEdgeType = findVariableNamesByEdgeType(viewDeclarationQuery,newEdgeType);

            if(Objects.equals(typeTable.get(viewName), "NODE")){
                for(String edgeName: variableNamesForTheEdgeType){
                    findNewMatchQuery = viewDeclarationQuery + " AND ID(" + edgeName + ")=" + newEdgeId+
                            " RETURN DISTINCT ID(" + viewReturnVarTable.get(viewName)+ ")";
                    HashMap<String,List<Integer>> newNodeids = connector.executeQuery(findNewMatchQuery);
                    HashMap<String, HashMap<String,Integer>> newlyAddedIdCount = connector.getNodeIDStringCount();
                    HashMap<String, HashMap<String,Integer>> originalIdCountSet = nodeCountTable.get(viewName);
                    ...
                    for (Map.Entry<String, HashMap<String,Integer>> newlyAddedCount : newlyAddedIdCount.entrySet()) {
                        String varName = newlyAddedCount.getKey();
                        HashMap<String,Integer> IdAndCount = newlyAddedCount.getValue();
                        HashMap<String,Integer>varMap = originalIdCountSet.get(varName);
                        for (Map.Entry<String, Integer> IdAndCountEntry:IdAndCount.entrySet()){
                            if(varMap.containsKey(IdAndCountEntry.getKey()))
                                varMap.put(IdAndCountEntry.getKey(),varMap.get(IdAndCountEntry.getKey())+IdAndCountEntry.getValue());
                            else
                                varMap.put(IdAndCountEntry.getKey(),IdAndCountEntry.getValue());
                        }
                    }
            ...
            }else if(Objects.equals(typeTable.get(viewName), "PATH")){
                for(String edgeName: variableNamesForTheEdgeType){
                    findNewMatchQuery = viewDeclarationQuery + " AND ID(" + edgeName + ")=" + newEdgeId+
                            " RETURN " + viewReturnVarTable.get(viewName);
                    Object[] processedPath = connector.pathQuery(findNewMatchQuery);
                    ...
                    for (Map.Entry<String, List<List<Integer>>> additionalPathSet : allrlist.entrySet()) {
                        String varName = additionalPathSet.getKey();
                        List<List<Integer>> newSet = additionalPathSet.getValue();
                        List<List<Integer>> newPathRelList =pathRelTable.get(viewName).get(varName);
                        newPathRelList.addAll(newSet);
                        pathRelTable.get(viewName).put(varName,newPathRelList);
                    }

                    HashMap<String,List<Integer>> edgeids = new HashMap<String,List<Integer>>();
                    for(Map.Entry<String,Set<Relationship>> newEdges : allRelationshipSet.entrySet()){
                        String pathName = newEdges.getKey();
                        Set<Relationship> relationshipSet = newEdges.getValue();
                    ...
                    // Storing edges
                    for (Map.Entry<String, List<Integer>> additionalEdgeSet : edgeids.entrySet()) {
                        String varName = additionalEdgeSet.getKey();
                        List<Integer> newSet = additionalEdgeSet.getValue();
                        List<Integer> newEdgeList =edgeTable.get(viewName).get(varName);
                        newEdgeList.removeAll(newSet);
                        newEdgeList.addAll(newSet);
                        edgeTable.get(viewName).put(varName,newEdgeList);
                    }

                    HashMap<String,List<Integer>> nodeids = connector.getNodeSet();
                    for (Map.Entry<String, List<Integer>> additionalNodeSet : nodeids.entrySet()) {
                        String varName = additionalNodeSet.getKey();
                        List<Integer> newSet =additionalNodeSet.getValue();
                        List<Integer> newNodeList = nodeTable.get(viewName).get(varName);
                        newNodeList.removeAll(newSet);
                        newNodeList.addAll(newSet);
                        nodeTable.get(viewName).put(varName,newNodeList);
                    }
                    HashMap<String, HashMap<String,Integer>> newlyAddedIdCount = connector.getNodeIDStringCount();
                    HashMap<String, HashMap<String,Integer>> originalIdCountSet = nodeCountTable.get(viewName);
                    for (Map.Entry<String, HashMap<String,Integer>> newlyAddedCount : newlyAddedIdCount.entrySet()) {
                    ...
                    System.out.println("updated duration: " + (end-start));
                    int count=0;
                    for(Map.Entry<String, List<List<Integer>>> paths: pathRelTable.get(viewName).entrySet()){
                        count += paths.getValue().size();
                    }
                    System.out.println("updated size: " + count);
                }
            }else if(Objects.equals(typeTable.get(viewName), "TABLE")){
                for(String edgeName: variableNamesForTheEdgeType){
                    String returnVarId ="";
                    for(String s: viewReturnVarTable.get(viewName).split(",")){returnVarId+= " ID("+s +"),";}
                    findNewMatchQuery = viewDeclarationQuery + " AND ID(" + edgeName + ")=" + newEdgeId+
                            " RETURN " +  returnVarId.substring(0,returnVarId.length()-1);
                    Object[] processedRecords = connector.generateTableView(findNewMatchQuery);
                    long executeEnd = System.currentTimeMillis();

                    List<List<Integer>> newlyAddedRecords = (List<List<Integer>>) processedRecords[0];
                    HashMap<String,List<Integer>> nodeids = (HashMap<String,List<Integer>>)processedRecords[1];
                    List<List<Integer>> originalRecords = recordTable.get(viewName);
                    originalRecords.addAll(newlyAddedRecords);
                    recordTable.put(viewName, originalRecords);
                    ...
                    long end = System.currentTimeMillis();
                    System.out.println("updated duration: " + (end-start));
                    int count=recordTable.get(viewName).size();
                    System.out.println("updated size: " + count);
                }
            }
    ...
    public static void updateViewWhenDeletingAnEdge(String command){
        long start = System.currentTimeMillis();
        String removedEdgeName = command.split("DELETE")[1].trim();
        String removedEdgeType = command.split(removedEdgeName+":")[1].split("]")[0];
        String EdgeIdtoRemove = connector.updateAnEntity(command.replace("DELETE","RETURN ID(") + ")");
        String findSuspiciouMatchQuery = "";
        String findStillValidMatchQuery = "";

        for(Map.Entry<String, String> entry : viewQueryTable.entrySet()){
            String viewName = entry.getKey();
            String viewDeclarationQuery = entry.getValue();
            int sizeRemoved = 0;
            List<String> variableNamesForTheEdgeType = findVariableNamesByEdgeType(viewDeclarationQuery,removedEdgeType);

            if(Objects.equals(typeTable.get(viewName), "NODE")){
                for(String edgeName: variableNamesForTheEdgeType){
                    String pattern = viewDeclarationQuery.split("MATCH")[1].split("WHERE")[0];
                    String[] conditions =  viewDeclarationQuery.split("WHERE")[1].split("AND");
                    String updatedCondition = viewDeclarationQuery.split("WHERE")[1];

                    findSuspiciouMatchQuery = viewDeclarationQuery +  " AND ID(" + edgeName + ")=" + EdgeIdtoRemove+
                            " RETURN ID(" + viewReturnVarTable.get(viewName)+ ")";
                    connector.executeQuery(findSuspiciouMatchQuery);
                    HashMap<String, HashMap<String,Integer>> suspiciousIdCount = connector.getNodeIDStringCount();
                    HashMap<String,List<Integer>> viewNodeSet = nodeTable.get(viewName);
                    HashMap<String,List<Integer>> updatedNodeSet = new HashMap<>();
                    HashMap<String, HashMap<String,Integer>> originalIdCountSet = nodeCountTable.get(viewName);

                    for(Map.Entry<String, HashMap<String, Integer>> nameAndId:suspiciousIdCount.entrySet()){
                        String keyName = nameAndId.getKey();
                        List<Integer> originalNodeSet = viewNodeSet.get(keyName);
                        HashMap<String,Integer> affectedIdCount = nameAndId.getValue();
                        HashMap<String,Integer> originalIdCount = originalIdCountSet.get(keyName);
                        for(Map.Entry<String,Integer> idAndCount: affectedIdCount.entrySet()){
                            if(idAndCount.getValue() == originalIdCount.get(idAndCount.getKey())){
                                originalNodeSet.remove(Integer.valueOf(Integer.parseInt(idAndCount.getKey())));
                                sizeRemoved+=1;
                            }
                        }
                        updatedNodeSet.put(keyName,originalNodeSet);
                    }
                    nodeTable.put(viewName,updatedNodeSet);

                    connector.executeQuery("MATCH ()-[r]-() WHERE ID(r)=" + EdgeIdtoRemove + " DELETE r");
                    long end = System.currentTimeMillis();
                    System.out.println("total duration: " + (end-start));
                    System.out.println("size removed: " + sizeRemoved);

                }
            }else if(Objects.equals(typeTable.get(viewName), "PATH")){
                for(String edgeName: variableNamesForTheEdgeType) {
                    String findInvalidPathQuery = viewDeclarationQuery + " AND ID(" + edgeName + ")=" + EdgeIdtoRemove +
                            " RETURN " + viewReturnVarTable.get(viewName);
                    Object[] processedPath = connector.pathQuery(findInvalidPathQuery);
                    HashMap<String, Set<Relationship>> allRelationshipSet = (HashMap<String, Set<Relationship>>) processedPath[0];
                    HashMap<String, List<List<Integer>>> allrlist = (HashMap<String, List<List<Integer>>>) processedPath[1];
                    HashMap<String, List<Integer>> suspiciousNodeids = connector.getNodeSet();
                    ...
                    for (Map.Entry<String, List<List<Integer>>> additionalPathSet : allrlist.entrySet()) {
                        ...
                        List<List<Integer>> newSet = additionalPathSet.getValue();
                        List<List<Integer>> newPathRelList = pathRelTable.get(viewName).get(varName);
                        newPathRelList.removeAll(newSet);
                        pathRelTable.get(viewName).put(varName, newPathRelList);
                    }

                    //in order to update the node table and edge table, we have to follow as the NODE VIEWS
                    HashMap<String, HashMap<String,Integer>> suspiciousIdCount = connector.getNodeIDStringCount();
                    HashMap<String,List<Integer>> viewNodeSet = nodeTable.get(viewName);
                    HashMap<String,List<Integer>> updatedNodeSet = new HashMap<>();
                    HashMap<String, HashMap<String,Integer>> originalIdCountSet = nodeCountTable.get(viewName);
                    ...
                    HashMap<String, HashMap<String,Integer>> suspiciousEdgeIdCount = connector.getEdgeIDStringCount();
                    HashMap<String,List<Integer>> viewEdgeSet = edgeTable.get(viewName);
                    HashMap<String,List<Integer>> updatedEdgeSet = new HashMap<>();
                    HashMap<String, HashMap<String,Integer>> originalEdgeIdCountSet = edgeCountTable.get(viewName);

                    for(Map.Entry<String, HashMap<String, Integer>> nameAndId:suspiciousEdgeIdCount.entrySet()){
                        String keyName = nameAndId.getKey();
                        List<Integer> originalEdgeSet = viewEdgeSet.get(keyName);
                        HashMap<String,Integer> affectedIdCount = nameAndId.getValue();
                        HashMap<String,Integer> originalIdCount = originalEdgeIdCountSet.get(keyName);
                        for(Map.Entry<String,Integer> idAndCount: affectedIdCount.entrySet()){
                            if(idAndCount.getValue() == originalIdCount.get(idAndCount.getKey())){
                                originalEdgeSet.remove(Integer.valueOf(Integer.parseInt(idAndCount.getKey())));
                            }
                        }
                        updatedEdgeSet.put(keyName,originalEdgeSet);
                    }
                    edgeTable.put(viewName,updatedEdgeSet);
                ...
                for(String edgeName: variableNamesForTheEdgeType) {
                    ...
                    String findInvalidPathQuery = viewDeclarationQuery + " AND ID(" + edgeName + ")=" + EdgeIdtoRemove +
                            " RETURN " + returnVarId.substring(0,returnVarId.length()-1);
                    Object[] processedRecords = connector.generateTableView(findInvalidPathQuery);
                    List<List<Integer>> suspiciousRecords = (List<List<Integer>>) processedRecords[0];

                    List<List<Integer>> originalRecords = recordTable.get(viewName);
                    originalRecords.removeAll(suspiciousRecords);
                    recordTable.put(viewName, originalRecords);
    ...
    public static String appendToSpecificEntity(String query,String suffix,String entity) {
        String updatedQuery ="";
        String remainingQuery = query;
        while(remainingQuery.contains("[")){
            String nameAndType = remainingQuery.split("\\[")[1].split("]")[0];
            if(nameAndType.contains(":") && nameAndType.substring(0, nameAndType.indexOf(":")).equals(entity))
                nameAndType = nameAndType.substring(0,nameAndType.indexOf(":"))+suffix + ":" + nameAndType.substring(nameAndType.indexOf(":")+1);
            else if(!nameAndType.contains(":") && nameAndType.equals(entity))
                nameAndType = nameAndType +suffix;
            updatedQuery = updatedQuery + remainingQuery.substring(0,remainingQuery.indexOf("["))+ "["+nameAndType + "]";
            remainingQuery = remainingQuery.substring(remainingQuery.indexOf("]")+1);
        }

        remainingQuery= updatedQuery+remainingQuery;
        updatedQuery="";
    ...
    public static String appendToEveryEntity(String query,String suffix) {
        String updatedQuery ="";
        String remainingQuery = query;
        while(remainingQuery.contains("[")){
            String nameAndType = remainingQuery.split("\\[")[1].split("]")[0];
            if(nameAndType.contains(":"))
                nameAndType = nameAndType.substring(0,nameAndType.indexOf(":"))+suffix + ":" + nameAndType.substring(nameAndType.indexOf(":")+1);
            else nameAndType = nameAndType +suffix;
            updatedQuery = updatedQuery + remainingQuery.substring(0,remainingQuery.indexOf("["))+ "["+nameAndType + "]";
            remainingQuery = remainingQuery.substring(remainingQuery.indexOf("]")+1);
        }
    ...
    public static String getRidOfNodeType(String query,String nodeName) {
        String updatedQuery ="";
        String remainingQuery = query;
        while(remainingQuery.contains("(")){
            String nameAndType = remainingQuery.split("\\(")[1].split("\\)")[0];
            if(nameAndType.contains(nodeName)) remainingQuery = remainingQuery.replace(nameAndType,nodeName);
            updatedQuery = updatedQuery + remainingQuery.substring(0,remainingQuery.indexOf(")")) + ")";
            remainingQuery = remainingQuery.substring(remainingQuery.indexOf(")")+1);
        }
        return updatedQuery;
    }

    public static List<String> findVariableNamesByEdgeType(String query,String edgeType) {
        List<String> validEdges = new ArrayList<>();
        String remainingQuery = query;
        while(remainingQuery.contains("[")){
            String nameAndType = remainingQuery.split("\\[")[1].split("]")[0];
            remainingQuery = remainingQuery.substring(remainingQuery.indexOf("]")+1);
            if(nameAndType.contains(edgeType)) validEdges.add(nameAndType.split(":")[0]);
        }
        return validEdges;
    }

    public static List<String> findVariableNamesByNodeType(String query,String nodeType) {
        List<String> validNodes = new ArrayList<>();
        String remainingQuery = query;
        while(remainingQuery.contains("(")){
            String nameAndType = remainingQuery.split("\\(")[1].split("\\)")[0];
            remainingQuery = remainingQuery.substring(remainingQuery.indexOf(")")+1);
            if(nameAndType.contains(nodeType)) validNodes.add(nameAndType.split(":")[0]);
        }
        return validNodes;
    }

    public static List<String> findAllVariableNames(String query) {
        List<String> allVarNames = new ArrayList<>();
        String remainingQuery = query;
        while(remainingQuery.contains("[")){
            String nameAndType = remainingQuery.split("\\[")[1].split("]")[0];
            remainingQuery = remainingQuery.substring(remainingQuery.indexOf("]")+1);
            allVarNames.add(nameAndType.split(":")[0]);
        }
    ...
    public static List<String> findAllNodeVariableNames(String query) {
        List<String> allVarNames = new ArrayList<>();
        String remainingQuery = query;
        while(remainingQuery.contains("(")){
            String nameAndType = remainingQuery.split("\\(")[1].split("\\)")[0];
    ...
    public static String getRidOfCOLLECT(String query){
        String beforeReturn = query.split("RETURN")[0];
        String afterReturn = query.split("RETURN")[1];
        afterReturn = afterReturn.replaceAll("COLLECTSET\\(([^)]+)\\)", "$1");
        return beforeReturn + " RETURN " + afterReturn;
    }
    ...
    public static void processMainView(String cmd, boolean materialized){
        ...
        if(vql.getReturnType() != QueryParser.retType.TABLE && vql.getReturnType() != QueryParser.retType.GRAPH ){
            fullQuery= getRidOfCOLLECT(fullQuery);
        }
        ...
        switch(vql.getReturnType()){
            case TABLE:{
                ...
                if(afterOrder == null)
                    makeMiddlewareView = mainQuery + "RETURN " + updateReturnClause.toString();
                else
        ...
        if(vql.getReturnType() == QueryParser.retType.TABLE) {

            Object[] processedResult = connector.generateTableView(makeMiddlewareView);
            recordTable.put(viewname, (List<List<Integer>>) processedResult[0]);
            nodeTable.put(viewname, (HashMap<String,List<Integer>>)processedResult[1]);
            viewReturnVarTable.put(viewname, (String) processedResult[2]);
            HashMap<String, HashMap<String,Integer>> nodeIdCount = connector.getNodeIDStringCount();
            nodeCountTable.put(viewname,nodeIdCount);
        ...
        if(vql.getReturnType() == QueryParser.retType.NODE) {
            ...
            HashMap<String,List<Integer>> nodes = connector.getNodeQuery(makeMiddlewareView);
        ...
        if(vql.getReturnType() == QueryParser.retType.PATH ){
            ...
            HashMap<String, HashMap<String,Integer>> nodeIdCount = connector.getNodeIDStringCount();
            nodeCountTable.put(viewname,nodeIdCount);
            HashMap<String, HashMap<String,Integer>> edgeIdCount = connector.getEdgeIDStringCount();
            edgeCountTable.put(viewname,edgeIdCount);
    ...
    public static long processUseViewRecord(String cmd){
        ...
        if(!cmd.contains("=*") && !vql.addWhereClauseRecord.keySet().isEmpty()) {
            for (String viewName : vql.addWhereClauseRecord.keySet()) {
                Map<String,String> usageNameToVarNameInView = vql.addWhereClauseRecord.get(viewName);
                String recConditionToReplace = vql.recordConditions.get(viewName);
                for (Map.Entry<String, String> entry : usageNameToVarNameInView.entrySet()) {
                    String nameInUsage = entry.getKey();
                    String nameInView = entry.getValue();
                    List<String> returnsOftheView = List.of(viewReturnVarTable.get(viewName).split(","));
                    int indexOfNameInView = returnsOftheView.indexOf(nameInView);
                    appendedToQuery = appendedToQuery  + "ID(" + nameInUsage  + ") = records["+ indexOfNameInView + "] AND ";
                }
                fullQuery = fullQuery.replace(recConditionToReplace,appendedToQuery.substring(0,appendedToQuery.length()-5));
                nodelist = recordTable.get(viewName);
                params.put("records",nodelist);
            }
            String newQuery ="UNWIND $records AS records " + fullQuery;
            try (Transaction tx = connector.getTransaction()) {
                long nowstart = System.currentTimeMillis();
                Result result = connector.executeWithParam(newQuery,params,tx);
                System.out.println("Took " + (System.currentTimeMillis() - nowstart) + " ms to execute transaction");
                Map<String, Object> row;
                int numResults = 0;
                HashMap<String,List<Integer>> nodeids = new HashMap<String,List<Integer>>();
    ...
    public static long processUseView(String cmd){
        ...
        if(vql.getViewScope()) {
		    ...
		    if(fullQuery.contains("WHERE"))
	                ...
	                List<Integer> ids = new ArrayList<>();
	           		...
	           		String replacement = "ID(" + nodeName + ") IN $" + viewName.split("\\.")[0];
	           		fullQuery = fullQuery.split("RETURN")[0] + "WHERE "+ replacement +" RETURN" + fullQuery.split("RETURN")[1];
	            }else if(!fullQuery.contains("*")){
                    	String subQuery = fullQuery;
                    	String newQuery = "";
                    	String appendedToQueryVql = "";
			    ...
			    if(!fullQuery.contains("WHERE"))
                                fullQuery = fullQuery.split("RETURN")[0]+" WHERE " + pathReplacement.substring(0, pathReplacement.length() - 5)+ "RETURN "+ fullQuery.split("RETURN")[1];
                            else
                                fullQuery = fullQuery.split("RETURN")[0]+" AND " + pathReplacement.substring(0, pathReplacement.length() - 5)+ "RETURN "+ fullQuery.split("RETURN")[1];
	    ...
	    for (String nodeName : vql.addWhereClause.keySet()) {
                for (String viewName : vql.addWhereClause.get(nodeName)) {
                    if(!viewName.startsWith("V")) continue;
                	String target = nodeName + " IN " + viewName;
                	String replacement = "";
                    String pathPattern = "";
		    if(Objects.equals(nodeName, "p")) {
                        ...
                        if(pathPattern.contains(","))
                            pathPattern= pathPattern.split("p=")[1].split(",")[0];
                    }

                	if(!fullQuery.contains(target)) continue;
                    if(Objects.equals(viewReturnVarTable.get(viewName.split("\\.")[0]), "GRAPH")){
                        List<Integer> allNodeIds = new ArrayList<>();
                        List<Integer> allEdgeIds = new ArrayList<>();
                        HashMap<String,List<Integer>> allIdsTemp = null;
                        allIdsTemp = nodeTable.get(viewName.split("\\.")[0]);
			...
			List<String> allVarName = findAllVariableNames(pathPattern);
                        for(String identifier: distinctNodeIdentifiers){
                            if(!Objects.equals(pathPattern, "") && !allVarName.contains(identifier)) continue;
                            whereCondition +=  "ID(" + identifier + ") IN $" + viewName.split("\\.")[0]+"Node AND ";
                        }
                    ...
                    }else if(!typeTable.get(viewName.split("\\.")[0]).equals("PATH")){
                        if(viewName.contains(".")) {
                            if(vql.nodeSymbols().contains(nodeName)){
                                List<Integer> ids = nodeTable.get(viewName.split("\\.")[0]).get("ID("+viewName.split("\\.")[1]+")");
                                idMap.put(viewName.split("\\.")[0] + "Node"  + viewName.split("\\.")[1], ids);
