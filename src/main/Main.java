        ...
        for(Map.Entry<String, String> entry : viewQueryTable.entrySet()){
            ...
            if(Objects.equals(typeTable.get(viewName), "NODE")){
                for(String edgeName: variableNamesForTheEdgeType){
                    ...
                    List<String> allVarNames = findAllVariableNames(pattern);
                    for(String varName : allVarNames){
                        if(!Objects.equals(varName, viewReturnVarTable.get(viewName))){
                            pattern=pattern.replace(varName+":",varName+"n:");
                            for(String condition:conditions){
                                if(condition.contains(varName+".") || condition.contains("ID("+varName+")"))
                                    updatedCondition += " AND "+condition.replace(varName,varName+"n");
                            }
                        }else pattern = getRidOfNodeType(pattern,varName);
                    }
                    findStillValidMatchQuery = viewDeclarationQuery.split("WHERE")[0]+ ", "+ pattern
                            + " WHERE " + updatedCondition + " AND ID(" + edgeName + ")=" + EdgeIdtoRemove+
                            " RETURN DISTINCT ID(" + viewReturnVarTable.get(viewName)+ ")";
                    HashMap<String,List<Integer>> suspiciousNodeids = connector.executeQuery(findSuspiciouMatchQuery);
                    HashMap<String,List<Integer>> stillValidNodeids = connector.executeQuery(findStillValidMatchQuery);

                    int newCountUnderAllnames = 0;
                    for (Map.Entry<String, List<Integer>> additionalNodeSet : suspiciousNodeids.entrySet()) {
                        String varName = additionalNodeSet.getKey();
                        List<Integer> suspiciousSet = additionalNodeSet.getValue();
                        List<Integer> stillValidSet = stillValidNodeids.get(varName);
                        if(stillValidSet != null)
                            suspiciousSet.removeAll(stillValidSet);
                        List<Integer> originalset = nodeTable.get(viewName).get(varName);
                        originalset.removeAll(suspiciousSet);
                        nodeTable.get(viewName).put(varName,originalset);
                        newCountUnderAllnames += originalset.size();
                    }
                    long end = System.currentTimeMillis();
                ...
                for(String edgeName: variableNamesForTheEdgeType) {
                    ...
                    String originalPattern = viewDeclarationQuery.split("MATCH")[1].split("WHERE")[0];
                    String pattern = originalPattern;
                    originalPattern = originalPattern.substring(originalPattern.indexOf(viewReturnVarTable.get(viewName) + "=")+viewReturnVarTable.get(viewName).length()+1);
                    String[] conditions = viewDeclarationQuery.split("WHERE")[1].split("AND");
                    String updatedCondition = viewDeclarationQuery.split("WHERE")[1];
                    List<String> allNodeNames = findAllNodeVariableNames(originalPattern);
                    List<String> allVarNames = findAllVariableNames(originalPattern);
                    originalPattern = appendToEveryEntity(originalPattern,"_o");

                    for (String anchorName : allNodeNames) {
                        pattern = viewDeclarationQuery.split("MATCH")[1].split("WHERE")[0];
                        conditions = viewDeclarationQuery.split("WHERE")[1].split("AND");
                        for (String varName : allVarNames) {
                            if (Objects.equals(varName, anchorName)) {
                                pattern = appendToSpecificEntity(pattern,"_o",varName);
                                for (String condition : conditions) {
                                    if (condition.contains(varName + ".") || condition.contains("ID(" + varName + ")")){
                                        updatedCondition += " AND " + condition.replace(varName, varName + "_o");
                                    }
                                }
                            }
                        }

                        //because of the rewritten, some var names in the condition might be obsolete, and we need to allow
                        // paths sharing edges
                        conditions = updatedCondition.split("AND");
                        String updateOriginalPatternThatNotHaveUnnecessaryEdge = "";
                        String[] updateOriginalPatternSections = originalPattern.split("\\(");
                        for(String section:updateOriginalPatternSections){
                            if(section.contains("[") && !section.contains(anchorName) && !section.contains(edgeName)) continue;
                            else if(!section.isEmpty()) updateOriginalPatternThatNotHaveUnnecessaryEdge += "("+section;
                        }
                        findStillValidMatchQuery = "MATCH" + pattern+ ", " + updateOriginalPatternThatNotHaveUnnecessaryEdge + " WHERE ";
                        for(String condition:conditions){
                            String varName = "";
                            if (condition.contains(".")) varName = condition.split("\\.")[0].trim();
                            else if (condition.contains("ID(")) varName=condition.split("ID\\(")[1].split("\\)")[0].trim();
                            if(pattern.contains("("+varName+")") ||pattern.contains("("+varName+":") ||
                                    updateOriginalPatternThatNotHaveUnnecessaryEdge.contains("("+varName+":")||updateOriginalPatternThatNotHaveUnnecessaryEdge.contains("("+varName+")"))
                                findStillValidMatchQuery += condition + " AND ";
                        }
                        findStillValidMatchQuery = findStillValidMatchQuery.substring(0,findStillValidMatchQuery.length()-5)
                         + " AND ID(" + edgeName + "_o)=" + EdgeIdtoRemove + " RETURN DISTINCT ID(" + anchorName+ "_o)";


                        HashMap<String,List<Integer>> stillValidNodeids = connector.executeQuery(findStillValidMatchQuery);
                        List<Integer> allStillValidNodeids = new ArrayList<>();
                        for (Map.Entry<String, List<Integer>> validIdEntry :stillValidNodeids.entrySet()) {
                            allStillValidNodeids.addAll(validIdEntry.getValue());
                        }
                        for (Map.Entry<String, List<Integer>> additionalNodeSet : suspiciousNodeids.entrySet()) {
                            List<Integer> suspiciousSet = additionalNodeSet.getValue();
                            if(!allStillValidNodeids.isEmpty())
                                suspiciousSet.removeAll(allStillValidNodeids);
                        }
                    }
                    ...
                    System.out.println("updated size: " + count);
                    System.out.println("updated node set size: " + newCountUnderAllnames);
                    connector.executeQuery(command);
                }
            }
