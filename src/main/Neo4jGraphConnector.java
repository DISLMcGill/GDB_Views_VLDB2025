...
public class Neo4jGraphConnector {
    ...
    public HashMap<String, HashMap<String,Integer>> nodeIDStringCount = new HashMap<>();
    public HashMap<String, HashMap<String,Integer>> edgeIDStringCount = new HashMap<>();
    ...
    public HashMap<String, HashMap<String,Integer>> getNodeIDStringCount(){
        HashMap<String, HashMap<String,Integer>> deepCopyMap = new HashMap<>();
        for (Map.Entry<String, HashMap<String, Integer>> entry : nodeIDStringCount.entrySet()) {
            String outerKey = entry.getKey();
            HashMap<String, Integer> innerMap = entry.getValue();

            HashMap<String, Integer> newInnerMap = new HashMap<>();
            for (Map.Entry<String, Integer> innerEntry : innerMap.entrySet()) {
                newInnerMap.put(innerEntry.getKey(), innerEntry.getValue());
            }

            deepCopyMap.put(outerKey, newInnerMap);
        }
        return deepCopyMap;
    }

    public HashMap<String, HashMap<String,Integer>> getEdgeIDStringCount(){
        HashMap<String, HashMap<String,Integer>> deepCopyMap = new HashMap<>();
        for (Map.Entry<String, HashMap<String, Integer>> entry : edgeIDStringCount.entrySet()) {
            String outerKey = entry.getKey();
            HashMap<String, Integer> innerMap = entry.getValue();
    ...
    public Object[] generateTableView(String query){
        //the first  element of result will be the records, and the second will be all nodes without duplicates
        // the third element will be a string of the real return order of elements
        ...
        if(query.equals("")) return resultObj;
        ...
        try( Transaction tx = db.beginTx()) {
            ...
            while (result.hasNext()) {
                ...
                for (Map.Entry<String, Object> column : row.entrySet()) {
                    oneRecord.add(Integer.parseInt(column.getValue().toString()));
                    List<Integer> oneSet;
                    ...
                    if(firstRecord){
                        returnOrder += column.getKey().toString().split("\\(")[1].split("\\)")[0].trim() + ",";
                    }
                }
                countRecord++;
            ...
            nodeIDStringCount.clear();
            for(Map.Entry<String,List<Integer>> entry: nodeids.entrySet()) {
                List<Integer> nodesWithDuplicates = entry.getValue();
                HashMap<String,Integer> countUnderAName = new HashMap<>();
                for (Integer number : nodesWithDuplicates) {
                    String numberString = number.toString();
                    countUnderAName.put(numberString, countUnderAName.getOrDefault(numberString, 0) + 1);
                }
                nodeIDStringCount.put(entry.getKey(),countUnderAName);
                nodeids.put(entry.getKey(), new ArrayList<>(
                        new HashSet<>(nodesWithDuplicates)));
            }
            resultObj[0]= records;
            resultObj[1]= nodeids;
            resultObj[2]= returnOrder;
            endTime = System.currentTimeMillis();
            executionTime = endTime - startTime;
            System.out.println("While Loop: " + executionTime + " milliseconds");
            System.out.println("There are " + countRecord + " records");
    ...
    public HashMap<String,List<Integer>> getNodeQuery(String query){
        HashMap<String,List<Integer>> nodeids = new HashMap<String,List<Integer>>();
        int numResults = 0;
        try( Transaction tx = db.beginTx()) {
            ...
            System.out.println("Took " + duration + " ms to retrieve");
            now = System.currentTimeMillis();
            ...
            System.out.println("Took " + duration + " ms to remove duplicates");
...
//System.gc();
    ...
    public String updateAnEntity(String query) {
        String Id = "";
        try( Transaction tx = db.beginTx()) {
            Result result = tx.execute( query );
            Map<String, Object> row;
            while (result.hasNext()){
                row = result.next();
                for (Map.Entry<String, Object> column : row.entrySet()) {
                   Id = String.valueOf(column.getValue());
                }
            }
            tx.commit();
        }
...
//    public  HashMap<String,List<Integer>>  executeLocalSearch(String query) {
//        HashMap<String,List<Integer>> nodeids = new HashMap<String,List<Integer>>();
//
//        try( Transaction tx = db.beginTx()) {
//            Result result = tx.execute( query );
    ...
    public HashMap<String,List<Integer>> relationshipNodes( HashMap<String,Set<Relationship>> relationshipSet){
        ...
        for(Map.Entry<String,Set<Relationship>> entry : relationshipSet.entrySet()){
        	...
        	for(Relationship r : relations){
                ...
                List<Integer> edgeidPath;
                if(edgeids.containsKey(pathName))
                    edgeidPath = edgeids.get(pathName);
                else
                    edgeidPath = new ArrayList<>();
                edgeidPath.add((int)r.getId());
                edgeids.put(pathName,edgeidPath);
        	}
        }
        ...
        for(Map.Entry<String,List<Integer>> entry: edgeids.entrySet()) {
            List<Integer> edgesWithDuplicates = entry.getValue();
            HashMap<String,Integer> edgeCountUnderAName = new HashMap<>();
            for (Integer number : edgesWithDuplicates) {
                String numberString = number.toString();
                edgeCountUnderAName.put(numberString, edgeCountUnderAName.getOrDefault(numberString, 0) + 1);
            }
            edgeIDStringCount.put(entry.getKey(),edgeCountUnderAName);
        }
